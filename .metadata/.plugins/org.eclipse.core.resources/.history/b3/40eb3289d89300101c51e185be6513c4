 ...
 * Le constructeur que j'utilise est defini dans la classe Monchien (donc pas ici)
 * 
 * Avec LES VALEURS que j'ai donnée a bill , bill devient une instance concrete de la
 * classe Monchien 
 * 
 * 
 * RESUMER
 * Main = classe qui lance ton programme

main() = point d’entrée

Création d’objet : Monchien bill = new Monchien(...)

Appel de méthodes sur l’objet : bill.aboyer(), bill.carteidentite()

La classe Monchien définit les caractéristiques et actions de l’objet, mais tout est utilisé ici dans Main.
 */